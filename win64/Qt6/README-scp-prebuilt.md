...
Build and install to prebuilt repo (with "x64 Native Tools Command Prompt"):
```bash
...
robocopy install <prebuilt-repo>\win64\Qt6 /e /move
```

Post-install cleanup:
```bash
cd /d <prebuilt-repo>\win64\Qt6
del /q bin\assistant.*
rmdir /s /q bin\Debug
del /q lib\*.prl
for /f "delims=" %G in ('dir /b /s /ad "objects-*"') do rmdir /s /q "%G"
robocopy . <out-of-repo-path>\win64\Qt6-pdbs *.pdb /s /mov
